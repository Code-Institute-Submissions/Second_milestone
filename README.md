...
I have decided to combine two example briefs and create a single page application that relies heavily on APIs including Google Maps API and to create a data visualization, that is API sourced and displayed using D3.js library.
...
- As a user _B_, I want a daily forecast showing wave and wind conditions.
...
- As a user _H_, an experienced surfer, I want an average forecast values for quick surf condition estimate that I could later explore in detail on other platforms or evaluate based on my own experience and knowledge.
- As a user _I_, a beginner surfer, I want a simple and clear average forecast values to tell me whether there are waves or not.
- As a user _J_, I want to see clear distinguish between Cardinal Directions of the surf spots to quickly identify locations of my interest, whether they are on the North, East, South or West of Ireland.
- As a user _K_, I want to see all available surfing spots for selected Cardinal Direction.

### Structure and Skeleton

Due to the application requirements and characteristics, the site design should be minimal in its form. The way in which the data is provided should lack any clutter but be informative as much as possible at the same time. There shouldn't be any distractions to the user so the focus should be primarily  on the forecast. The website's simplicity and its responsive structure should provide functionality and intuitive interactions for the users.
...
- The map sourced from Google Maps API is styled to reflect the overall design approach. The map features and elements like labels, roads, water, etc. are either minimalized or removed in order to avoid any distraction to the user and make it simple. Furthermore, a custom markers were added to the map and styled accordingly to the site to make the location of Surfing Spots clearly visible.
...
- "MENU" button to make a shortcuts to the selection of Cardinal Directions.
- Interactive Google Map to show the exact Surfing Spots locations.
...
- Direct link to the forecast from the Google Maps infoWindow. 
...
4. User "E" wants to see a simplified wind forecast that takes into account the wind direction and Surfing Spot point. The simplified wind forecast should state whether the wind type is Offshore, Onshore or Cross-shore wind. The following steps needs to be conduct in that case:
    1. Select a Cardinal Direction of your choice, e.g. "West".
    2. Select a Surfing Spot title of your choice, e.g. "Lahinch".
    3. The wind forecast can be found on cards with "WAVE" and "WIND" description. The wind type is displayed on the left hand side of the "WIND" text and is in a form of a text range including "OFF" for Offshore wind, "ON" for Onshore wind and "CROSS" for Cross-shore wind.
...
6. User "J" wants a clear distinguish between Cardinal Directions of the Surfing Spots while user "K" wants to see all available Surfing Spots for selected Cardinal direction. The following steps needs to be conduct in that case:
    1. Navigate to the bottom of the page where four buttons with Cardinal Directions are located. Each of the buttons once engaged, will display a list of buttons with Surfing Spots Titles for selected Cardinal Direction.


#### Manual Testing Bugs

The one major issue that I have encountered during the test was the limit on daily requests for one of my initial API's. The StormGlass Api, which was later replaced with a limit free Marine Institute's API, had a limit of 50 requests per day. Taking into consideration the amount of Surfing Spots, totalling to 20, I would limit the application to serve just couple of users at best scenario.  
...
Jasmine is a behaviour-driven development framework for testing JavaScript code. My approach to the Jasmine testing was to breakdown large functions and to test them separately. There are 37 specs tested with no failures.

In order to run the Jasmine test please open the "specRunner.html" file in a browser. The test results will be displayed immediately. The source code and test specifications can be found in the "js" folder.
